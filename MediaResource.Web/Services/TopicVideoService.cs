    ...
    public class TopicVideoService : IDisposable
    {
        ...
        public StaticPagedList<ImageViewModel> AdvancedSearch(int? topicId, int? nodeId, int? userPlateId, string keyword, int? pageSize, int? pageIndex, Dictionary<string, string> searchCondition)
        {
            ...
            if (nodeId != null)
            {
                ...
                {
                    var theNode = node;
                    nodeCondition = nodeCondition.Or(i => ("|" + i.Nodes + "|").Contains("|" + theNode.NodeName.Trim() + "(" + theNode.Id + ")|"));
                }
            ...
            if (userPlateId != null)
            {
                ...
                query = query.Where(i => ("|" + i.Nodes + "|").Contains("|" + userPlate.PlateName.Trim() + "(" + userPlate.Id + ")|"));
            }
		...
		public void PostScore(int topicVideoId, double scoreValue, int scoreCount)
		{
			TopicVideo topicVideo = _db.TopicVideos.Find(topicVideoId);
			topicVideo.Score = scoreValue;
			topicVideo.ScoreCount = scoreCount;
			_db.Entry(topicVideo).State = EntityState.Modified;
			_db.SaveChanges();
		}
